...
|-|-|-|
| inc | Инкремент | acc + 1 -> acc |
| dec | Декримент | acc - 1 -> acc |
| add | Сумма acc и mem | acc + mem -> acc |
| sub | Разница acc и mem| acc - mem -> acc |
| mul | Произведение acc и mem | acc * mem -> acc |
| div | Частное acc и mem| acc / mem -> acc |
| mod | Остаток от деления acc и mem | acc % mem -> acc |
| mov | Загрузить в acc адрес метки | addr -> acc |
| load | Загрузить в acc из памяти по адресу метки | [addr] -> acc |
| store | Загрузить в память по адресу метки из acc | acc -> [addr] |
| ld | load с косвенной адресацией | [[addr]] -> acc |
| st | store с косвенной адресацией | acc -> [[addr]]|
| jmp | бесусловный переход | addr -> pc|
| jz | переход, если acc = 0 | если acc = 0, то addr -> pc |
| jnz | переход, если acc != 0 | если acc != 0, то addr -> pc |
| halt | остановка | остановка выполнения программы |
